    ...
    for (;;) {
        ...
        while (audio_pkt_size > 0) {
            ...
            data_size = av_samples_get_buffer_size(NULL, aCodecCtx->channels, frame.nb_samples,
                                                   static_cast<AVSampleFormat>(AUDIO_S16SYS), 1);
            ...
            ALOGI("frame.sample_rate = %d \n", frame.sample_rate);
            ALOGI("frame.format = %d \n", frame.format);
            ALOGI("frame.format bits = %d \n", av_get_bytes_per_sample(
                    static_cast<AVSampleFormat>(frame.format)));
            ALOGI("frame.channels = %d \n", frame.channels);
            ALOGI("frame.channel_layout = %d \n", frame.channel_layout);
            ALOGI("frame.nb_samples = %d \n", frame.nb_samples);
            ALOGI("\n");
            ...
            if (swr_ctx) {
                ...
                resampled_data_size = wanted_frame.channels * len2 * av_get_bytes_per_sample(
                        (enum AVSampleFormat) wanted_frame.format);
            } else {
...
int main(int argc, char *argv[]) {
    ...
    ALOGI("codec request_sample_fmt= %d", aCodecCtx->request_sample_fmt);
...
//    wanted_spec.format = AUDIO_S16SYS;
    ...
    wanted_spec.samples = static_cast<Uint16>(aCodecCtx->sample_rate / aCodecCtx->channels);
    wanted_spec.callback = audio_callback;
    wanted_spec.userdata = aCodecCtx;

    ALOGI("codecCtx sample_rate = %d \n", aCodecCtx->sample_rate);
    ALOGI("codecCtx channels = %d \n", aCodecCtx->channels);
    ALOGI("codecCtx sample_fmt = %d \n", aCodecCtx->sample_fmt);
    ALOGI("AUDIO_S16SYS = %d \n", AUDIO_S16SYS);
    ALOGI("\n");
    ...
    ALOGI("spec freq = %d \n", spec.freq);
    ALOGI("spec format = %d \n", spec.format);
    ALOGI("spec channels = %d \n", spec.channels);
    ALOGI("spec samples = %d \n", spec.samples);
    ALOGI("spec silence = %d \n", spec.silence);
    ALOGI("spec padding = %d \n", spec.padding);
    ALOGI("spec size = %d \n", spec.size);
    ALOGI("\n");

    ALOGI("AV_SAMPLE_FMT_S16 = %d \n", AV_SAMPLE_FMT_S16);
    //因为SDL中的音频数据没有AV_SAMPLE_FMT_S16P，这种planer的方式的，所以在播放的时候，如果取到这个数据，还需要进行转换
